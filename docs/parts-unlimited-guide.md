...
4. **ソースコード取得**: GitHub から ZIP ダウンロード → 展開
   - `.csproj` のターゲット フレームワークを .NET 4.5.1 から **.NET 4.8 に変更**します。Windows Server 2022 には .NET 4.8 が標準搭載されていますが、4.5.1 Targeting Pack は含まれないためです。アプリは .NET 4.8 で完全に動作します (上位互換)
...
8. **SQL Server 接続確認**: `Test-NetConnection` で DB01 (10.0.1.5) の TCP 1433 への疎通を確認し、結果を表示します。失敗した場合は DB01 側の `Setup-SqlServer.ps1` の実行状況やファイアウォール設定を確認してください
...
> **初回アクセス時の DB 自動作成**: 初回アクセス時に Entity Framework Code First が DB01 (10.0.1.5) に接続し、データベースとテーブルを自動作成します。このため初回は表示に時間がかかる場合があります。DB 接続に失敗するとエラーになりますので、事前に DB01 側の SQL Server セットアップ (手順 1) が完了していることを確認してください。
