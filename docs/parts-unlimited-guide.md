...
Bastion RDP を使わず、ローカル PC から Azure CLI の `az vm run-command invoke` を使ってスクリプトを実行することも可能です。VM にスクリプトを事前配置する必要はなく、ローカルのファイル内容が VM に送信されて実行されます。
...
az vm run-command invoke `
  ...
  --scripts @scripts/Setup-SqlServer.ps1 `
...
az vm run-command invoke `
  ...
  --name OnPrem-Web `
  --command-id RunPowerShellScript `
  --scripts @scripts/Setup-PartsUnlimited.ps1 `
  --parameters "SqlPassword=<Setup-SqlServer.ps1 と同じパスワード>"
```

> **注意**: `Setup-PartsUnlimited.ps1` は VS Build Tools のインストールを含むため、完了まで 15〜20 分程度かかる場合があります。`az vm run-command invoke` はスクリプト完了まで待機し、実行結果を返します。
